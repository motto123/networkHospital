 ...
 */
package com.huiyuan.networkhospital.module.huanxin.widget;
...
public class PasteEditText extends EditText{
    ...
    public boolean onTextContextMenuItem(int id) {
        if(id == android.R.id.paste){
            ...
            if(text.startsWith(ChatMainActivity.COPY_IMAGE)){
//                intent.setDataAndType(Uri.fromFile(new File("/sdcard/mn1.jpg")), "image/*");     
                text = text.replace(ChatMainActivity.COPY_IMAGE, "");
                ...
                ((Activity)context).startActivityForResult(intent,ChatMainActivity.REQUEST_CODE_COPY_AND_PASTE);
//                clip.setText("");
            }
        }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        if(!TextUtils.isEmpty(text) && text.toString().startsWith(ChatMainActivity.COPY_IMAGE)){
            setText("");
        }
