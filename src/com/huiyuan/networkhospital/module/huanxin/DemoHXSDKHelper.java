 ...
 */
package com.huiyuan.networkhospital.module.huanxin;
...
import com.huiyuan.networkhospital.module.huanxin.receiver.CallReceiver;
		...
		eventListener = new EMEventListener() {
			...
			public void onEvent(EMNotifierEvent event) {
				...
				if(event.getData() instanceof EMMessage){
					...
					String custom=message.getStringAttribute("cmd",null);
					if(null != custom){
						NApplication.cmd=custom;
						
					}
					char role = message.getFrom().charAt(0);
					NApplication.role=role;
				}
			...
			public Intent getLaunchIntent(EMMessage message) {
				...
				if(isVideoCalling){
					...
					if (chatType == ChatType.Chat) { // 单聊信息
						...
						intent.putExtra("self", message.getTo().substring(1, 12));
						intent.putExtra("way", "notifyBar");
						intent.putExtra("chatType", ChatMainActivity.CHATTYPE_SINGLE);
					} else { // 群聊信息
						// message.getTo()为群聊id
						intent.putExtra("groupId", message.getTo());
						if(chatType == ChatType.GroupChat){
							intent.putExtra("chatType", ChatMainActivity.CHATTYPE_GROUP);
						}else{
							intent.putExtra("chatType", ChatMainActivity.CHATTYPE_CHATROOM);
						}
